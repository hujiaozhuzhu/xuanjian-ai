...
SQL_INJECTION_RULES = [
    PythonRule(
        ...
        severity="CRITICAL",
        confidence=0.8,
        code_pattern=r"""['\"]\s*(?:SELECT|INSERT|UPDATE|DELETE)\b[^'\"]*['\"]\s*\+""",
...
COMMAND_INJECTION_RULES = [
    PythonRule(
        ...
        severity="CRITICAL",
        confidence=0.85,
        code_pattern=r"""(?:os\.(?:system|popen)\s*\(\s*[A-Za-z_]\w*\s*\)|subprocess\.(?:call|Popen|check_output)\s*\(\s*[A-Za-z_]\w*\s*[,)]|subprocess\.run\s*\(\s*(?!\w+\s*\.split\s*\()[A-Za-z_]\w*\s*[,)]|(?:os\.system|os\.popen|subprocess\.(?:call|run|Popen|check_output))\s*\(\s*(?:f['\"]|['\"].*\+|['\"].*%))""",
...
CRYPTO_RULES = [
    PythonRule(
        ...
        false_positive_indicators=["checksum", "fingerprint", "hmac", "non_crypto"],
    ),
    PythonRule(
        ...
        severity="CRITICAL",
        confidence=0.7,
        code_pattern=r"""(?:SECRET|PASSWORD|TOKEN|API_KEY|ACCESS_KEY)[A-Z_]*\s*=\s*['\"][^'\"]{8,}['\"]""",
        category="SECRETS",
        cwe="CWE-798",
...
PATH_TRAVERSAL_RULES = [
    PythonRule(
        ...
        severity="HIGH",
        confidence=0.7,
        code_pattern=r"""(?:os\.path\.join\s*\([^)]*\b(?:file|filename|pathname|user_path)\b|open\s*\(\s*(?:filepath|file_path|user_path)\b)""",
