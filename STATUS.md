...
## v2.2.3 压缩 bundle 多命中定位

原始压缩产物通常全部处于第 1 行。v2.2.2 已提供格式化后位置，但 ScannerManager 和 ResultNormalizer 的去重逻辑会把同文件、同规则、同原始行的多处命中错误折叠。

v2.2.3 将去重和稳定指纹改为优先使用 `original_offset_hint.original_start`，缺失偏移时使用 `beautified_line`。这保证同一 bundle 内相同规则、相同代码片段的不同静态位置都会保留，同时普通源码仍按原始行去重。

**回归范围**: 2048 个重复 `eval()` 的压缩单行 bundle 必须保留 2048 个 ScanResult、2048 个 Finding、2048 个互异指纹；无动态执行、解密或写回行为。全量单元测试 `684 passed in 28.50s`，覆盖率 `68.94%`，高于门禁 `59%`。
...
| v2.2.3 | 2026-09-04 | 压缩 bundle 多命中去重与稳定指纹修复 | 684 | 68.94% |
